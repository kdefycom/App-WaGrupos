let meusGrupos = [];
let fotoFile = null; // Variável para o arquivo da foto no modal
...
function setupImagePreview() {
    ...
    fileInput.addEventListener("change", () => {
        ...
        fotoFile = file; // Armazena o arquivo para upload
...
async function carregarMeusGrupos() {
    ...
    try {
        ...
        const { data: gruposAPI, error } = await supabase.from('grupos').select('*').in('id', ids);
...
function renderizarGrupo(grupo) {
    // ... (função de renderização permanece a mesma)
...
function editarGrupo(id) {
    ...
    document.getElementById('editDescricao').value = grupo.descricao || ''; // Popular descrição
    ...
    fotoFile = null; // Limpa o arquivo de foto anterior ao abrir o modal
...
async function salvarEdicao() {
    ...
    const mudouDescricao = descricao !== grupo.descricao;
    const mudouFoto = fotoFile !== null;

    if (!mudouFoto && !mudouLink && !mudouDescricao) {
    ...
    try {
        ...
        const updateData = {
            ...
            descricao: descricao,
            foto_url: foto_final,
            aprovado: false, // Reanálise necessária
            mensagem_admin: 'Grupo em reanálise após edição.'
        };
        
        const { error } = await supabase.from('grupos').update(updateData).eq('id', id);
        if (error) throw error;
        ...
        popup(`Erro ao salvar: ${error.message}`);
    } finally {
...
async function impulsionar(event, id) { const button = event.target; button.disabled = true; button.textContent = 'IMPULSIONANDO...'; try { await supabase.from('grupos').update({ ultimo_boost: new Date().toISOString() }).eq('id', id); carregarMeusGrupos(); } catch { button.disabled = false; button.textContent = '🚀 IMPULSIONAR'; } }
async function removerGrupo(id) { if (!confirm('Tem certeza que deseja apagar este grupo?')) return; try { await removerGrupoLocal(id); await supabase.from('grupos').delete().eq('id', id); carregarMeusGrupos(); } catch(error) { console.error("Erro ao remover grupo:", error); } }
